# codex - ctxloom family library. No binary; the immutable git tag (driven by VERSION) is the reference.
# Library: no binary is produced; the immutable git tag (driven by VERSION) is
# the reference consumers pin (github.com/ctxloom/codex@v0.7.0-pre1 / @v0.7).
#
...
devcontainer_image := "codex-devcontainer"
